...
export class Area2D {
    ...
    static readonly N8 = [-1, 0, 1]
        .flatMap((offset_y) =>
            [-1, 0, 1].map((offset_x) => pos2D(offset_x, offset_y)),
        )
...
export class Array2D<T> {
    ...
    static fromSize<T>(size: Pos2D, initializer: (pos: Pos2D) => T) {
        return new Array2D<T>(
            [...Array(size.y)].map((_, y) =>
                [...Array(size.x)].map((__, x) => initializer(pos2D(x, y))),
            ),
        );
    }
    ...
    public isInside(point: Pos2D) {
        return (
            this.isInRange(point.x, 0, this.size.x) &&
            this.isInRange(point.y, 0, this.size.y)
        );
    }
    ...
    public map<U>(operation: Morphism<T, U>) {
        return new Array2D(
            this.array.map((line, j) =>
                line.map((cell, i) => operation(cell, new Pos2D(i, j), this)),
            ),
        );
    }
    ...
    public mapRow<U>(operation: LineMorphisn<T, U>) {
        return new Array2D(
            this.array.map((line, j) => operation(line, j, this)),
        );
    }

    public mapColumn<U>(operation: LineMorphisn<T, U>) {
        return new Array2D(
            Array2D.transposeArray(
                Array2D.transposeArray(this.array).map((line, j) =>
                    operation(line, j, this),
                ),
            ),
        );
    }
    ...
    public forEach<U>(operation: Operation<T>) {
        this.array.forEach((line, j) =>
            line.forEach((cell, i) => operation(cell, new Pos2D(i, j), this)),
        );
    }

    public every(predicate: Predicate<T>) {
        return this.array.every((line, j) =>
            line.every((cell, i) => predicate(cell, new Pos2D(i, j), this)),
        );
    }

    public some(predicate: Predicate<T>) {
        return this.array.some((line, j) =>
            line.some((cell, i) => predicate(cell, new Pos2D(i, j), this)),
        );
    }
    ...
    public findIndexAll(predicate: Predicate<T>): Pos2D[] {
        return this.map((c, p, a) => (predicate(c, p, a) ? [p] : []))
            .getRawArray()
            .flatMap((x) => x)
            .flatMap((x) => x);
    }
    ...
    public neighbourhood(center: Pos2D, neighbourhood: Pos2D[]) {
        return Area2D.neighbourhood(center, neighbourhood).filter((p) =>
            this.isInside(p),
        );
    }
